...
const Statistics: React.FC<StatisticsProps> = ({ trades, lang }) => {
  ...
  const { stats, equityCurve, dailyPnl, fullPeriodBalance } = useMemo(() => {
    ...
    // 1. Calculate Full Period balance (Always from Start to Now)
    ...
    // 2. Filter trades based on date range for the Metrics & Curve
    ...
    // 3. Calculate balance at the start of the filtered range
    ...
    // 4. Build Equity Curve for filtered range
    ...
    // 5. Daily PNL map for heatmap (showing only filtered)
  ...
  return (
    <div className="p-6 space-y-10 max-w-4xl mx-auto pb-32 animate-in fade-in duration-700">
      <section className="space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
           <div className="animate-in slide-in-from-left-4">
              ...
              <div className="flex items-baseline gap-3 mt-3">
                <span className="text-6xl font-black font-mono tracking-tighter transition-all">
        ...
        {/* Quick Range Buttons (7d, 30d, 90d, 180d, All Time) */}
        ...
        <div className="grid grid-cols-2 gap-4 p-4 bg-[#0A0A0A] border border-[#1A1A1A] rounded-3xl animate-in fade-in duration-1000">
           <div className="space-y-1">
              <label className="text-[9px] font-black text-zinc-600 uppercase tracking-widest">{t.fromDate}</label>
              <input 
                type="date" 
                value={fromDate} 
                onChange={e => { setFromDate(e.target.value); setActiveQuickRange('Custom'); }}
                className="w-full bg-black border border-zinc-800 rounded-xl px-4 py-2.5 text-xs text-white outline-none focus:border-[#00FFFF]"
                style={{ colorScheme: 'dark' }}
              />
           </div>
           <div className="space-y-1">
              <label className="text-[9px] font-black text-zinc-600 uppercase tracking-widest">{t.toDate}</label>
              <input 
                type="date" 
                value={toDate} 
      ...
      <section className="bg-[#0A0A0A] border border-[#1A1A1A] rounded-[2.5rem] p-8 space-y-6 shadow-2xl">
...
const MetricCard: React.FC<{ label: string; value: string; color: string }> = ({ label, value, color }) => (
  <div className="bg-[#0A0A0A] border border-[#1A1A1A] rounded-[2rem] p-8 space-y-1 shadow-xl">
