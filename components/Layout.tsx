...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, lang }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-black text-white font-sans selection:bg-[#00FFFF] selection:text-black">
      <header className="px-6 py-8 flex items-center gap-5 max-w-4xl mx-auto w-full animate-in fade-in duration-1000">
        {/* Logo 容器 - 完全依照右圖風格設計 */}
        <div className="w-14 h-14 border border-zinc-800 flex items-center justify-center bg-zinc-950 rounded-xl overflow-hidden shrink-0 shadow-[0_0_30px_rgba(0,0,0,0.5)]">
          <img 
            src={MH_LOGO_PATH} 
            alt="MH" 
            className="w-full h-full object-cover"
            onError={(e) => {
              // 如果圖片失效，直接顯示右圖那樣的藍綠色 MH 文字圖標
              const target = e.currentTarget;
              target.style.display = 'none';
              const parent = target.parentElement;
              if (parent) {
                parent.innerHTML = '<div class="text-[#00FFFF] font-black text-sm tracking-tighter">MH</div>';
              }
            }}
          />
        </div>
        ...
        <div className="space-y-0.5 overflow-hidden">
          <h1 className="text-lg font-black tracking-tighter uppercase flex items-center gap-1.5 whitespace-nowrap">
            MH <span className="text-[#00FFFF] drop-shadow-[0_0_8px_rgba(0,255,255,0.4)]">TRADING</span> JOURNAL PRO
          </h1>
          <p className="text-[8px] text-zinc-600 font-black uppercase tracking-[0.5em] truncate opacity-80">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-black/80 backdrop-blur-3xl border-t border-zinc-900/50 px-8 pt-4 pb-10 z-50">
...
const NavButton: React.FC<{ active: boolean; onClick: () => void; label: string; icon: React.ReactNode }> = ({ active, onClick, label, icon }) => (
  <button onClick={onClick} className={`flex flex-col items-center gap-1.5 transition-all ${active ? 'text-[#00FFFF]' : 'text-zinc-700 hover:text-zinc-500'}`}>
    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 ${active ? 'bg-[#00FFFF]/10 ring-1 ring-[#00FFFF]/40 shadow-[0_0_30px_rgba(0,255,255,0.15)]' : ''}`}>
      {icon}
    </div>
